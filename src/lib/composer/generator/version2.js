...
const RHYTHMS_SLOW = Array(Array('h', 'h'), Array('q', 'q', 'h'), Array('h', 'q', 'q'), Array('q', 'q', 'q', 'q'));
const RHYTHMS_FAST = Array(
  Array('16', '16', '16', '16', '16', '16', '16', '16', '16', '16', '16', '16', '16', '16', '16', '16'),
  Array('e', 'e', '32', '32', '32', '32', '32', '32', '32', '32', 'e', 'e', '16', '16', '16', '16'),
  Array('16', '16', '16', '16', '32', '32', '32', '32', '32', '32', '32', '32', 'e', 'e', 'e', 'e'),
  Array('e', '16', '16', 'e', '16', '16', '16', '16', '16', '16', '32', '32', '32', '32', 'e')
);
const RHYTHMS_BALANCED = Array(
  Array('e', '16', '16', 'e', 'e', '16', '16', 'q'),
  Array('q', 'e', '16', '16', 'h'),
...
export function generateMeasure(currentScore) {
  ...
  if (currentScore && currentScore.trim().length > 0) {
    ...
    if (notes.length > 0) {
      // Filter out leading rest elements to read the final actual pitch reference in the history chain
      const lastPitchToken = notes.filter(n => !n.toUpperCase().startsWith('R')).at(-1);
      
      if (lastPitchToken) {
        const targetNote = lastPitchToken.split('/').at(0).trim();
        const match = targetNote.match(/([A-G])(#|b)?(\d)/);
        if (match) {
          const letter = match.at(1);
          const acc = match.at(2) || '';
  ...
  const rootMidi = PITCHES[keyCenter];
  const scaleIntervals = isMinor ? Array(0, 2, 3, 5, 7, 8, 11) : Array(0, 2, 4, 5, 7, 9, 11);
  ...
  for (let i = 0; i < activeRhythm.length; i++) {
    ...
    // --- NEW: SYSTEM REST ALLOCATION ENGINE GENERATOR ---
    // Rule 1: Structural down-beats (i === 0) never clear into rests to anchor chords
    // Rule 2: 20% random evaluation chance for balanced and fast note divisions
    const isRestChance = i > 0 && Math.random() < 0.20;

    if (isRestChance) {
      outputTokens.push(`R/${duration}`);
      // Crucial: Skip rewriting 'lastMidi' so the tracking thread is preserved for the next actual pitch
    } else {
      lastMidi = idealMidi; // Only advance voice pitch values when notes are explicitly produced
      const activeAlphabet = useFlats ? ALPHABET_FLATS : ALPHABET_SHARPS;
      const finalLetter = activeAlphabet.at(lastMidi % 12);
      const finalOctave = Math.floor(lastMidi / 12) - 1;
      outputTokens.push(`${finalLetter}${finalOctave}/${duration}`);
